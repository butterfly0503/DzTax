...
import {handleRenewSubscription} from '../../func/userSettings'
import FlashMessage from "react-native-flash-message";

class RenewSubscriptionPayment extends React.Component {
  constructor(props) { 
    super(props);
...
async handlePayment(){
    if(this.state.chosenPayment!=''){
        let {months}=this.props.route.params
        let resp=await handleRenewSubscription(this.props.user.credentials.login,this.props.user.credentials.password,months)
        if(resp.success){
    ...
    return(
        <View style={styles.pickingContainerStyle}>
            ...
            <View style={{...styles.textContainerStyle,width:'45%',borderRadius:10}}>
                <Text style={{fontSize:20}} numberOfLines={1} adjustsFontSizeToFit={true}>{this.props.route.params.months*100} zł</Text>
            </View>
        </View> 
    )
}
